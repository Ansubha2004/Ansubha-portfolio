...
export default function ContactPage() {
  ...
  const updatejson = (linkmap) => {
    contactdetails.forEach((data) => {
      if (data.id == 1) data.link = linkmap;
  ...
  const sociallinks = [
    "https://www.linkedin.com/in/ansubha-dhar-856176296/",
    "https://www.instagram.com/rikstar_bro/",
    "https://github.com/Ansubha2004",
  ];
  ...
  return (
    <div className="absolute top-0">
      <main className="h-screen w-screen flex justify-center items-center ">
        <div
          data-aos="zoom-in-up"
          className=" h-auto rounded-[20px] box-border p-[50px] w-[80%] bg-orange-200 flex mt-[25px]"
        >
          <div className="w-[50%]">
            <p className=" text-[#132238] text-[35px] font-bold font-[poppins]">
              Let's Connect & Create
            </p>
            <br />
            <p className="w-[90%] text-[#556070] text-[0.9rem]">
              I'd love to connect with you! Whether you have a project idea, a
            ...
            {contactdetails.map((Details, index) => (
              <div key={Details.id}>
                <a
                  target="_blank"
                  href={Details.link}
                  className="group transition-all ease-out duration-200 h-auto  w-[90%] rounded-[7px] shadow-none hover:shadow-[0px_0px_35px_rgba(0,0,0,0.2)] p-[15px] flex"
                >
                  <div
                    target="_blank"
                    className="h-auto w-auto p-[10px] rounded-[5px] text-red-500 bg-red-300 group-hover:bg-red-500  group-active:bg-red-800 group-hover:text-white"
                  >
                    {React.createElement(Connections[index], {
                      className: "h-[25px] w-[25px]",
                    })}
                  </div>
                  <div className="flex flex-col ml-3 font-[poppins] text-[15px] justify-center">
                    <p className=" text-[#556070] text-[0.9em]">
                      {Details.header}
                      <span> :</span>
                    </p>
                    <p className=" font-bold text-[1.1em] tracking-wider">
                      {Details.content}
                    </p>
                  </div>
                </a>
            ...
            <div className="flex w-full space-x-7 ml-2">
              {socialmedia.map((Component, index) => (
                <a
                  ...
                  target="_blank"
                  className="h-auto w-auto p-[15px] rounded-[5px] text-red-500 transition-all ease-out duration-200 hover:bg-red-500  active:bg-red-800 hover:text-white"
                >
                  <Component className="h-[30px] w-[30px] " />
                </a>
              ))}
            </div>
          </div>
          <div className="w-[50%] flex flex-col justify-center items-start">
            <p className="text-[#556070] text-[0.9rem]">
            ...
            <div className="w-full">
              <form onSubmit={handlesubmit}>
                {form.map((data, index) => (
                  <input
                    ...
                    key={data.id}
                    type={data.type}
                    className="h-[40px] focus:outline-none   w-full border-b-[1px] border-b-gray-500 border-b-solid border-box px-2 mb-5 hover:placeholder-red-600 hover:border-b-red-600 active:border-b-red-600"
                    name={data.name}
              ...
              <br />
                ...
                <button type="submit"  className="hover:cursor-pointer hover:text-red-800 font-bold px-[20px]  py-[10px]  rounded-[7px] bg-red-500 text-white transition-all duration-300 linear hover:bg-red-800">
                  Submit
                </button>
                </form>
            </div>
